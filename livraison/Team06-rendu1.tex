...
\paragraph{}
La classe \textt{Livre} représente une édition de livre alors qu'un exemplaire est un livre physique présent dans la bibliothèque. Cette relation est representée comme une association entre la classe \textt{Livre} et la classe \textt{Exemplaire de livre}. Un exemplaire n'est relié qu'à un seul livre mais un livre est relié à plusieurs exemplaires. Nous disposons d'un attribut calculé au sein du livre afin de connaitre le nombre d'exemplaires de ce livre présent dans la bibliothèque. La cote d'un exemplaire est representée par le numéro de document dont \textt{Exemplaire de livre} hérite. Les détails concernant l'emprunt et  la réservation se retrouvent dans les classes d'associations \textt{Emprunt} et \textt{Réservation}.La perte se retrouve directement au sein de l'exemplaire. La méthode \textt{est\_disponible()} de la classe exemplaire prend en considération les réservations, l'emprunt et la perte de l'exemplaire du livre.
...
\paragraph{}
La classe \textt{Emprunt} contient la date et le nombre de relances pour celui-ci, la relance étant gérée par le serveur de messages nous avons choisi de ne pas modéliser de classe Relance, mais de préciser ses opérations dans le diagramme de séquence "Relancer pour rendu du livre".
